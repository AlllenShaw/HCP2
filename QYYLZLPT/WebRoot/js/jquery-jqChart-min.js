...
(function(a) {
	...
	a.fn.jqChart.defaults = {
		...
		exportConfig : {//exportConfig revised
			server : "",
			method : ""
		}
	};
	...
	d.isPointerDevice = window.navigator.pointerEnabled
			|| window.navigator.msPointerEnabled;
	d.checkIfRetinaDisplay = function() {
		var a = "(-webkit-min-device-pixel-ratio: 1.5),                      (min--moz-device-pixel-ratio: 1.5),                      (-o-min-device-pixel-ratio: 3/2),                      (min-resolution: 1.5dppx)";
		return window.devicePixelRatio >= 2 ? true : window.matchMedia
	...
	d.prototype = {
		_createElements : function(c) {
			...
			if (d.isTouchDevice) {
				...
				a.each([ "pointerup", "pointercancel", "MSPointerUp",
		...
		_getLocOffset : function() {
			var b = this.elem.offset(), c = parseFloat(a(document.body).css(
					"borderLeftWidth"));
			if (!isNaN(c))
				b.left += c;
		...
		_setTooltipPos : function(e, j) {
			var i = this.tooltip.outerWidth(true), h = this.tooltip
					.outerHeight(true), g = this._width, f = this._height, d = e[0]
					._getTooltipPosition(j, i, h, g, f), c = d.y, k = e.length, l = this;
			if (k > 1) {
				c = 0;
				a.each(e, function() {
	...
	jb.prototype = {
		...
		_getIntervals : function(c, b) {
			...
			if (b && b.intervalOffset)
				d = b.intervalOffset;
			for ( var e = [], f = this._getIntervalStart(this
					._getActualVisibleMinimum(), c), a = f + d; a <= this
					._getActualVisibleMaximum(); a = this
		...
		_arrange : function() {
			...
			if (this.title.text)
				...
				case "left":
					...
					this.title.y = this.title.rotY = this.y
							+ (this.height + this.title.width) / 2;
		...
		_getMainLine : function() {
			var b = this.crossOffsetX, c = this.crossOffsetY, a = new i(this.x1
		...
		_getTickMarks : function(c, p) {
			if (c == null || c.visible != true)
				return [];
			for ( var n = [], g = this.crossOffsetX, h = this.crossOffsetY, k = this.offset, j = c.position == "inside", r = this
					._getMarkInterval(c, p), b = c.length, o = this
					._getIntervals(r, c, p), d, f, e, a, m = 0; m < o.length; m++) {
		...
		getZoom : function() {
			if (!this.actualMaximum)
				return 1;
			var b = this._getActualMaximum() - this._getActualMinimum(), a = this
					._getActualVisibleMaximum()
					- this._getActualVisibleMinimum(), c = a / b;
			return c
		},
		...
		getPosition : function(f) {
			var e = this._getActualVisibleMaximum(), d = this
					._getActualVisibleMinimum(), a = this.length / (e - d)
					* (f - d), b = this.reversed === true, c = this.isAxisVertical;
		...
		getValue : function(f) {
			var e = this._getActualVisibleMaximum(), b = this
					._getActualVisibleMinimum(), a = (f - this.origin)
					* (e - b) / this.length + b, c = this.reversed === true, d = this.isAxisVertical;
	...
	var n = -Number.MAX_VALUE, j = Number.MAX_VALUE;
	function b() {
	}
	b.isNull = function(b) {
	...
	b.intersection = function(a, c, b, d) {
		var f, h = (d.x - b.x) * (a.y - b.y) - (d.y - b.y) * (a.x - b.x), i = (c.x - a.x)
				* (a.y - b.y) - (c.y - a.y) * (a.x - b.x), e = (d.y - b.y)
				* (c.x - a.x) - (d.x - b.x) * (c.y - a.y);
		if (e != 0) {
			var g = h / e, j = i / e;
			f = {
				x : a.x + g * (c.x - a.x),
	...
	c.prototype.fillStyleToJSON = function(a) {
		if (typeof a == "string" || this.width == null || this.height == null
	...
	c.prototype._createGradient = function(j, e) {
		...
		if (e == null || typeof e == "string" || this.width == null
				|| this.height == null || c == null || d == null)
			return e;
		if (this.xOffset)
			c += this.xOffset;
		if (this.yOffset)
		...
		switch (e.type) {
		case "radialGradient":
			var m = c + b.x0 * this.width, o = d + b.y0 * this.height, k = b.r0
					* this.width / 2, n = c + b.x1 * this.width, p = d + b.y1
					* this.height, l = b.r1 * this.width / 2;
			f = j.createRadialGradient(m, o, k, n, p, l);
			break;
		default:
			var s = c + b.x0 * this.width, t = d + b.y0 * this.height, r = c
					+ b.x1 * this.width, q = d + b.y1 * this.height;
	...
	i.prototype.hitTestNonHV = function(n, o, l) {
		...
		var k = Math.sqrt(e * e + f * f), j = (e * n - f * o + i) / k, m = Math
				.max(l, 3);
	...
	i.prototype.render = function(a, j) {
		...
		if (this.rotationAngle && !b.isNull(this.rotX) && !b.isNull(this.rotY)) {
			var i = b.rotatePointsAt([ e, g, f, h ], this.rotationAngle,
					this.rotX, this.rotY);
			e = i[0];
			g = i[1];
			f = i[2];
	...
	i.prototype.toJSON = function() {
		var a = this, b = "type:'line',x1:" + a.x1 + ",y1:" + a.y1 + ",x2:"
				+ a.x2 + ",y2:" + a.y2;
	...
	l.prototype.render = function(a, k) {
		...
		var h = this.strokeStyle != null && this.lineWidth > 0, j = h
				&& Math.floor(this.lineWidth % 2) ? b.roundH : Math.round, i = this.correction || 0;
		i = Math.round(i);
		var f = j(this.x), g = j(this.y), e = Math.round(this.width), d = Math
				.round(this.height), m = this.context;
		if (this.xDecrease) {
			e -= this.xDecrease;
	...
	l.prototype.toJSON = function() {
		var a = this, b = "type:'rectangle',x:" + a.x + ",y:" + a.y + ",width:"
	...
	Bb.prototype.render = function(a, m) {
		...
		if (this.yOffset)
			e += this.yOffset;
		var g = this.width / 2 * .5522848, h = this.height / 2 * .5522848, i = b
				+ l, j = e + k, f = b + l / 2, d = e + k / 2;
		a.beginPath();
		a.moveTo(b, d);
	...
	Bb.prototype.toJSON = function() {
		var a = this, b = "type:'ellipse',x:" + a.x + ",y:" + a.y + ",width:"
				+ a.width + ",height:" + a.height;
		b += c.prototype.toJSON.call(this);
		return b
	};
	...
	Cb.prototype.toJSON = function() {
		var a = this, b = "type:'arc',x:" + a.x + ",y:" + a.y + ",radius:"
				+ a.radius + ",startAngle:" + a.startAngle + ",endAngle:"
				+ a.endAngle;
		b += c.prototype.toJSON.call(this);
		return b
	};
	...
	Y.prototype.toJSON = function() {
		var a = this, b = "type:'circle',x:" + a.x + ",y:" + a.y + ",radius:"
				+ a.radius;
		b += c.prototype.toJSON.call(this);
		return b
	};
	...
	vb.prototype.hitTest = function(g, h) {
		...
		var c = b.normalizeAngle(this.startAngle), d = b
				.normalizeAngle(this.endAngle);
	...
	vb.prototype.toJSON = function() {
		var a = this, b = "type:'pieSlice',x:" + a.x + ",y:" + a.y + ",radius:"
				+ a.radius + ",startAngle:" + a.startAngle + ",endAngle:"
				+ a.endAngle;
		b += c.prototype.toJSON.call(this);
		return b
	...
	z.prototype.hitTest = function(k, l, g) {
		...
		for ( var d = this.pts, m = Math.pow(g, 2), c = -1, f = j, h, i, b, n = this.isStepLine ? 4
				: 2, a = 0; a < d.length; a += n) {
			h = d[a];
			i = d[a + 1];
	...
	z.prototype.getCenter = function(g) {
		...
		for ( var b = this.pts, a = 0, f = j, i = g.locX, d, h = this.isStepLine ? 4
				: 2, c = 0; c < b.length; c += h) {
			d = Math.abs(b[c] - i);
			if (f > d) {
				f = d;
				a = c
	...
	q.prototype.renderPoints = function(a, c, i) {
		...
		for ( var h = this.nullHandling != "connect", e = true, b = 2; b < g; b += 2) {
			...
			if (d == null) {
				if (h) {
					this.strokeStyle != null && this.lineWidth > 0
							&& a.stroke();
					e = false
				}
				continue
	...
	bb.prototype.toJSON = function() {
		var a = this, b = "type:'area',pts:" + a.arrayToJSON(a.pts)
				+ ",crossPos:" + a.crossPos + ",vertical:" + a.vertical;
		if (a.isCurve)
			b += ",isCurve:" + a.isCurve;
		b += c.prototype.toJSON.call(this);
		return b
	...
	X.prototype.toJSON = function() {
		var a = this, b = "type:'rangeShape',pts1:" + a.arrayToJSON(a.pts1)
				+ ",pts2:" + a.arrayToJSON(a.pts2);
		if (a.isCurve)
			b += ",isCurve:" + a.isCurve;
		b += c.prototype.toJSON.call(this);
		return b
	...
	m.prototype.renderText = function(b, c, a) {
		...
		if (h) {
			var g = this.text.split(m.separator), d = this.getHeight(), e;
	...
	m.prototype.measure = function(e) {
		...
		var c = this.getHeight(), a, b, g = this.text
				&& this.text.search(m.separator) != -1;
		if (g) {
	...
	m.prototype.intersectWith = function(c, d, i, h) {
		var a = this.x, b = this.y, g = this.width, f = this.height, e = this
				._correctXY(a, b, g, f);
		a = e.x;
		b = e.y;
	...
	m.prototype.toJSON = function() {
		...
		if (!a.text || a.text == "")
			return false;
		var e = b.replaceTextForExport(this.text), d = "type:'textBlock',x:"
				+ a.x + ",y:" + a.y + ",width:" + a.width + ",height:"
				+ a.height + ",text:'" + e + "'";
		d += ",font:'" + a.font + "'";
	...
	ec.prototype.toJSON = function() {
		var a = this, b = "type:'clip',x:" + a.x + ",y:" + a.y + ",width:"
				+ a.width + ",height:" + a.height;
		return b
	};
	function bc() {
	...
	O.prototype._renderShadows = function(h) {
		...
		for ( var f = 0; f < h.length; f++) {
			var e = h[f];
			e && e.shadowColor && e.shadowColor != "rgba(0, 0, 0, 0)"
					&& e.render(a)
		}
		g && a.restore()
	};
	...
	wb.prototype._render = function(c) {
		var a = this.visible ? this.lineWidth / 2 : 0, f = this.x + a, g = this.y
				+ a, e = this.width - 2 * a, d = this.height - 2 * a, b = new l(
				f, g, e, d);
	...
	W.prototype.mouseUp = function() {
		var a = this.mouseInput1, b = this.view.mouseInput, e = Math.abs(a.x
				- b.x), f = Math.abs(a.y - b.y);
		if (e > 1 || f > 1) {
	...
	e.prototype._setOptions = function(h) {
		...
		c.mouseWheelInteractionMode = c.mouseWheelInteractionMode
		...
		var f = this._width = this.elem.width(), e = this._height = this.elem
	...
	e.prototype.isServerFiltering = function() {
		...
		return !b ? false : b.serverFiltering === true && a.isPlainObject(b)
				&& b.ajax ? true : false
	};
	e.prototype._processAsyncData = function() {
		if (!this.isServerFiltering())
			return;
		var d = this, g = d.options.dataSource;
		d.arrDataSource = null;
		var h = g.ajax, b = {};
		a.extend(b, h);
		b.url += "?";
	...
	e.prototype._arrange = function() {
		...
		d -= c.left + c.right;
		g -= c.top + c.bottom;
		var e = c.left, f = c.top + this.title.height, o = this.axes
				._getTotalWidth(), n = this.axes._getTotalHeight();
		...
		case "left":
			...
			this.legend.y = c.top + this.gridArea.y
	...
	e.prototype._initCrosshairs = function(d) {
		if (!this.hasCrosshairs)
			return;
		var h = this.options.crosshairs, c = {}, e = d.locX, f = d.locY;
	...
	e.prototype._renderSelectionRect = function(g, h) {
		...
		var m = Math.min(c, d), n = Math.min(e, f), k = Math.abs(c - d), j = Math
				.abs(e - f), i = new l(m, n, k, j);
	...
	e.prototype.toJSON = function(j) {
		var e = this.gridArea, d = this.options.shadows, f = d && d.enabled, i = new ec(
				e.x, e.y, e.width, e.height), h = new bc, d = new Wb(true), g = new Wb(
				false), c = this.allShapes, b = [];
		a.merge(b, c.shapes);
		if (this.hasData) {
			b.push(i);
			if (f) {
	...
	M.prototype.toJSON = function() {
		var a = this, b = "type:'stock',x:" + a.x + ",width:" + a.width
				+ ",high:" + a.high + ",low:" + a.low + ",open:" + a.open
				+ ",close:" + a.close;
	...
	hb.prototype.createElements = function(b, e, f, o, m, n) {
		var d = [], g = Math.floor(n / 2), b = Math.round(b), a = Math.round(o), c = Math
				.round(m);
		if (a > c) {
			var k = c;
			c = a;
	...
	hb.prototype.toJSON = function() {
		var a = this, b = "type:'candlestick',x:" + a.x + ",width:" + a.width
				+ ",high:" + a.high + ",low:" + a.low + ",open:" + a.open
				+ ",close:" + a.close;
		if (a.priceUpFillStyle)
			b += ",priceUpFillStyle:" + a.fillStyleToJSON(a.priceUpFillStyle);
		if (a.priceDownFillStyle)
			b += ",priceDownFillStyle:"
					+ a.fillStyleToJSON(a.priceDownFillStyle);
		b += c.prototype.toJSON.call(this);
		return b
	};
	function Vb(a) {
	...
	Yb.prototype = {
		...
		show : function() {
			this.toolbar && this.visibility == "auto"
	...
	kb.prototype._measure = function() {
		...
		var o = 2 * this.margin + e.left + e.right, q = 2 * this.margin + e.top
				+ e.bottom, k = this.chart.border.getPaddings(), m = this.chart._width
				- k.left - k.right - o;
		if (l && b > m) {
			var h = Math.floor(m / d) || 1, n = Math
					.ceil(this.items.length / h);
			this.rows = h;
			this.cellHeight = a + g;
			this.cellWidth = d;
			b = h * d;
	...
	B.prototype._arrangeRenderer = function(a) {
		if (!d.use_excanvas)
			return;
		var f = this.x, g = this.y, b = a.canvas, h = a.div, e = Math.max(
	...
	B.prototype._getStripes = function(b) {
		...
		a.each(d, function() {
			...
			if (a.width)
				...
				i = b.actualInterval;
			for ( var h = a.lineWidth, o = h / 2, n = b._getIntervals(j, a,
					true), m = 0; m < n.length; m++) {
				var k = n[m];
				if (k >= b.actualVisibleMaximum)
					continue;
				var d = b.getPosition(k), p = b._getNextPosition(k, i), f = b
						.getPosition(p), g;
				if (b.isAxisVertical)
					g = new l(c.x + o, Math.min(d, f), c.width - h, Math.abs(f
							- d));
				else
					g = new l(Math.min(d, f), c.y + o, Math.abs(f - d),
							c.height - h);
				a._setSettings(g);
				e.push(g)
			}
	...
	B.prototype._getGridLines = function(a, b, k) {
		if (a.location == "radial" || b == null || b.visible != true)
			return [];
		for ( var h = [], n = a._getMarkInterval(b, k), j = a._getIntervals(n,
				b, k), c, d, e, f, g = 0; g < j.length; g++) {
	...
	B.prototype._getPlotLines = function(h, o) {
		...
		for ( var e = this, k = [], l = [], j = 0; j < g.length; j++) {
			...
			if (b.fitInRange(d.zIndex, 0, 2) != o)
				continue;
			var f = d.lineWidth / 2, n = h.getPosition(d.value), c = new ab(
					d.title);
			c.chart = this.chart;
			c._measure();
			if (h.isAxisVertical) {
	...
	x.prototype._initRanges = function() {
		for ( var e = j, d = n, c = j, b = n, g = this.items, f = 0; f < g.length; f++) {
			var a = g[f];
			if (!a.isInScene())
				continue;
			if (e > a.min)
	...
	x.prototype._findStackedClusters = function(h, e) {
		for ( var f = h.stackedGroupName, g = this._getStackedGroupsFromType(e), l = a
				.inArray(f, g), k = g.length, i = -1, c = 0, j = this.items, d = 0; d < j.length; d++) {
			var b = j[d];
			if (!b.isInScene())
				continue;
			if (b == h)
	...
	f.prototype = {
		...
		_addMarker : function(d, e, l, c, f, k, g) {
			var a = this._correctMarkerPosition(d, e, c), h = this._getMarker(
					a.x, a.y, l, c, f, k, g), j = this.markers.offset, b = null;
			if (j) {
				b = new i(d, e, a.x, a.y);
		...
		_addMarkerAndLabel : function(i, p, l, m, a, k, e, f, o, c) {
			...
			if (this.markers && this.markers.isVisible()
				...
				var n = c.dataItem[2], d = this._addMarker(l, m, null, f, n, a,
						c);
				if (d.marker) {
		...
		_addShapeAnimation : function(e, g, f) {
			...
			if (!b || b.enabled === false)
				return;
			var a = new zb(b, e, "visible", false, true), c = a.duration / f, d = a.delayTime
					+ g * c;
			a.delayTime = d;
			a.duration = c;
		...
		_setShapeSettings : function(c, e) {
			...
			if (c.context && this.hasNullValues
		...
		_processData : function() {
			...
			if (this.data)
				...
				if (c) {
					var f = this.xValuesField, g = this.yValuesField, h = b
							.processDataField(c, f), e = b.processDataField(c,
							g);
					if (e)
		...
		_processDataXYZ : function() {
			this.arrData = null;
			if (this.data)
				if (!this.xValuesType && !this.fromValuesType
						&& !this.toValuesType)
					this.arrData = b.cloneArray(this.data);
				else {
					this.arrData = [];
				...
				var c = this.chart.arrDataSource;
				if (c) {
					var j = this.xValuesField, h = this.fromValuesField, i = this.toValuesField, g = b
							.processDataField(c, j), e = b.processDataField(c,
							h), f = b.processDataField(c, i);
					if (g && e && f)
		...
		_initXYData : function() {
			for ( var l = this.arrData, d = j, c = n, h = j, g = n, m = l.length, k = 0; k < m; k++) {
				var i = l[k];
				if (i == null)
					continue;
				var e = i[0];
		...
		_initCatValueData : function() {
			for ( var k = this.arrData, h = j, g = n, f = [], l = k.length, e = 0; e < l; e++) {
				var d = k[e];
				if (d == null) {
					f.push((e + 1).toString());
					continue
		...
		_initDateValueData : function() {
			for ( var l = this.arrData, e = j, d = n, i = j, h = n, m = l.length, f, a, c, k = 0; k < m; k++) {
				f = l[k];
				if (b.isNull(f))
					continue;
				a = f[0];
		...
		_initXYDataRange : function(m, p) {
			for ( var l = this.arrData, h = j, g = n, f = j, e = n, o = l.length, i = 0; i < o; i++) {
				var d = l[i];
		...
		_initCatValueDataRange : function(o, q, m) {
			for ( var k = this.arrData, g = j, f = n, d = [], p = k.length, e = 0; e < p; e++) {
				var h = k[e];
				if (h == null) {
					d.push((e + 1).toString());
					continue
		...
		_initDateValueDataRange : function(o, q) {
			for ( var m = this.arrData, i = j, h = n, g = j, f = n, p = m.length, k = 0; k < p; k++) {
				var e = m[k];
		...
		_initVisibleXYData : function() {
			if (this.realYAxis.zoomEnabled)
				return;
			var c = this.realXAxis, o = c.visibleMinimum || c.minimum, m = c.visibleMaximum
					|| c.maximum, k = !b.isNull(o), i = !b.isNull(m);
			if (!k && !i)
				return;
			for ( var l = this.arrData, e = j, d = n, q = l.length, h = 0; h < q; h++) {
				var g = l[h];
				if (g == null)
					continue;
				var a = g[1];
		...
		_initVisibleCatValueData : function() {
			if (this.realYAxis.zoomEnabled)
				return;
			var e = this.realXAxis, p = e.visibleMinimum || e.minimum, o = e.visibleMaximum
					|| e.maximum, l = !b.isNull(p), k = !b.isNull(o);
			if (!l && !k)
				return;
			for ( var m = this.arrData, g = j, f = n, q = m.length, d = 0; d < q; d++) {
				var h = m[d];
				if (h == null)
					continue;
				var c = h;
		...
		_initVisibleCatValueDataRange : function(p, r) {
			if (this.realYAxis.zoomEnabled)
				return;
			var d = this.realXAxis, m = d.visibleMinimum || d.minimum, l = d.visibleMaximum
					|| d.maximum, i = !b.isNull(m), h = !b.isNull(l);
			if (!i && !h)
				return;
			for ( var k = this.arrData, f = j, e = n, q = k.length, c = 0; c < q; c++) {
				var o = k[c];
				if (o == null)
					continue;
				if (i && c + 1 < m || h && c > l)
		...
		_initVisibleXYDataRange : function(q, s) {
			...
			if (!k && !i)
				return;
			for ( var l = this.arrData, e = j, d = n, r = l.length, g = 0; g < r; g++) {
				var f = l[g];
				if (f == null)
					continue;
				var p = f[0];
		...
		_initStackedData : function(b) {
			...
			if (a.isArray(c) == false)
				return;
			var f = this.chart.series._findStackedClusters(this, b), e = this.chart.series
					._getStackedSeriesFromType(b, this.stackedGroupName), d = this
					._calcStackedData(c, f, e);
			a.extend(this, d)
		},
		_initVisibleStackedData : function(b) {
			...
			var f = this.chart.series._findStackedClusters(this, b), e = this.chart.series
					._getStackedSeriesFromType(b, this.stackedGroupName), d = this
					._calcVisibleStackedData(c, f, e);
			a.extend(this, d)
		},
		_calcStackedData : function(s, u, t) {
			for ( var m = j, l = n, k = j, i = n, g = [], v = s.length, q = {}, f = 0; f < v; f++) {
				var c = s[f];
				if (c == null) {
					g.push((f + 1).toString());
					continue
				...
				for ( var r = this._getXValue(c, f), b = {
					positive : 0,
					negative : 0
				}, e = null, o = u.index - 1; o >= 0; o--) {
		...
		_calcVisibleStackedData : function(s, w, u) {
			...
			if (!g)
				return;
			var l = g.visibleMinimum || g.minimum, k = g.visibleMaximum
					|| g.maximum, i = !b.isNull(l), h = !b.isNull(k);
			if (!i && !h)
				return;
		...
		_calcColumnScale : function(m) {
			...
			var c = this.realXAxis, l = c.series, o = b.isNull(c.minimum) ? l.minX
					: c.minimum, n = b.isNull(c.maximum) ? l.maxX : c.maximum, i = n
					- o;
			if (c.skipEmptyDays)
				i -= c.totalEmptyDaysTicks;
			i = Math.min(.5, k / i);
			return i
		},
	...
	A.prototype._renderCatData = function(n) {
		for ( var o = this.arrData, p = o.length, f = this.markers != null
				&& this.markers.isVisible(), c = [], i = [], m = [], k = [], q = f ? this.markers.size / 2
				: 0, h, j, r, d, e = 0; e < p; e++) {
			var g = o[e];
			if (g === null) {
				c.push(null);
	...
	A.prototype._renderLinearData = function(j) {
		var q = this.arrData, k = q.length, d = this.markers != null
		...
		for ( var c = [], h = [], p = [], n = [], s = d ? this.markers.size / 2
				: 0, l, m, f, e, g = 0; g < k; g++) {
			var i = q[g];
			if (i === null) {
				c.push(null);
	...
	A.prototype._renderLargeXYData = function(t) {
		for ( var n = this.arrData, x = n.length, w = this.chart.gridArea.width, s = this.chart.gridArea.height, i = this.realXAxis, j = this.realYAxis, u = 2
				* (i.actualVisibleMaximum - i.actualVisibleMinimum) / w, v = 2
				* (j.actualVisibleMaximum - j.actualVisibleMinimum) / s, a = [], q, r, c, b, o = 0, p = 0, k = 0, l = 0, d, f, g, e = [], h = 0; h < x; h++) {
			d = n[h];
			if (d === null) {
				a.push(null);
				a.push(null);
	...
	eb.prototype._createShape = function(i, f) {
		...
		var g = this.chart.gridArea, k = g.y, j = g.y + g.height, e = this.realYAxis
				.getCrossingPosition();
	...
	s.prototype._renderCatData = function(p) {
		var r = this.arrData, i = this.chart.gridArea, z = i.x, E = z + i.width, e = this.realYAxis
				.getCrossingPosition();
		e = b.fitInRange(e, z, E);
		e = Math.round(e);
		var q = this.chart.series._findClusters(this, this.type), m = r.length, C = i.height
				/ this.realXAxis.getZoom(), x = C / m, F = x / q.count, f = Math
				.round(this.pointWidth * F), B = q.count * f, o = (x - B) / 2;
		o = Math.round(o + q.index * f);
		f = Math.max(f, this.minimumWidth);
		for ( var v = [], s = [], D = this.markers && this.markers.isVisible() ? this.markers.size / 2 + 2
				: 2, c, k, d = 0; d < m; d++) {
			var n = r[d];
			if (n == null)
				continue;
	...
	s.prototype._renderLinearData = function(n) {
		var o = this.arrData, g = this.chart.gridArea, x = g.x, E = x + g.width, d = this.realYAxis
				.getCrossingPosition();
		d = b.fitInRange(d, x, E);
		d = Math.round(d);
		var p = o.length, A = g.height / this.realXAxis.getZoom(), D = this
				._getSeriesFromThisType(), C = this._calcColumnScale(D) * A, m = this.pointWidth
				* C;
		m = Math.max(m, this.minimumWidth);
		for ( var u = [], r = [], B = this.markers && this.markers.isVisible() ? this.markers.size / 2
				: 0, c, q, i, j, k, e = 0; e < p; e++) {
			k = o[e];
	...
	s.prototype._getDataPointLabel = function(k, g, l, f, h) {
		var i = k <= this.realYAxis.getCrossing(), j = this._getLabelText(h), b = new m(
				j);
		d.setShadows(b, this, this.chart);
		a.extend(b, this.labels);
		b.measure(this.chart.ctx);
	...
	cb.prototype._processData = function() {
		...
		if (this.data)
			...
			var c = this.chart.arrDataSource;
			if (c) {
				var i = this.xValuesField, j = this.yValuesField, h = this.sizeValuesField, f = b
						.processDataField(c, i), g = b.processDataField(c, j), e = b
						.processDataField(c, h);
				if (f && g && e)
	...
	cb.prototype._initData = function() {
		...
		if (a.isArray(c) == false)
			return;
		for ( var k = j, i = n, h = j, g = n, e = j, d = n, q = c.length, b = 0; b < q; b++) {
			var m = c[b][0], o = c[b][1], f = c[b][2];
			if (m == null || o == null || f == null)
				continue;
			h = Math.min(h, m);
			g = Math.max(g, m);
			k = Math.min(k, o);
	...
	cb.prototype._render = function(m) {
		...
		for ( var i = h.length, j, k, e, c, b, a = 0; a < i; a++) {
			...
			if (this.labels && this.labels.visible !== false) {
				var s = this._getLabelValue(b, a), n = this._getDataPointLabel(
						b, j, k, q / 2, s);
				m.push(n);
	...
	J.prototype._renderCatData = function(p) {
		var r = this.arrData, j = this.chart.gridArea, C = j.y, B = j.y
				+ j.height, d = this.realYAxis.getCrossingPosition();
		d = b.fitInRange(d, C, B);
		d = Math.round(d);
		var q = this.chart.series._findClusters(this, this.type), k = r.length, y = j.width
				/ this.realXAxis.getZoom(), v = y / k, A = v / q.count, f = Math
				.round(this.pointWidth * A), x = q.count * f, o = (v - x) / 2;
		o = Math.round(o + q.index * f);
		f = Math.max(f, this.minimumWidth);
		for ( var u = [], s = [], z = this.markers && this.markers.isVisible() ? this.markers.size / 2
				: 0, g, h, e, c = 0; c < k; c++) {
			var m = r[c];
			if (m == null)
				continue;
	...
	J.prototype._renderLinearData = function(n) {
		var o = this.arrData, j = this.chart.gridArea, B = j.y, z = j.y
				+ j.height, c = this.realYAxis.getCrossingPosition();
		c = b.fitInRange(c, B, z);
		c = Math.round(c);
		var p = o.length, w = j.width / this.realXAxis.getZoom(), A = this
				._getSeriesFromThisType(), y = this._calcColumnScale(A) * w, i = this.pointWidth
				* y;
		i = Math.max(i, this.minimumWidth);
		for ( var s = [], q = [], x = this.markers && this.markers.isVisible() ? this.markers.size / 2
				: 0, e, f, u, g, d = 0; d < p; d++) {
			var k = o[d];
			if (k == null || a.isArray(k) == false)
				continue;
	...
	h.prototype._processData = function() {
		...
		if (this.data) {
			...
			var c = this.chart.arrDataSource;
			if (c) {
				var e = this.dataLabelsField, f = this.dataValuesField, g = b
						.processDataField(c, e), h = b.processDataField(c, f);
	...
	h.prototype._createLabels = function() {
		...
		if (!B)
			return f;
		var l = this.chart.gridArea, J = l.width, I = l.height, A = this
				._getYValues(), D = A.length, E = this.getTotal(), q = this.margin, g = this
				._calcRadius({
					h : q,
					v : q
				});
		if (g < 0)
			return f;
		var r = l.x + J / 2, s = l.y + I / 2, H = Math.PI * 2 / E, o = b
				.radians(this.startAngle), t = this.explodedRadius;
		if (!this.labels || this.labels.visible === false)
			return f;
		for ( var k, y, p = [], o = b.radians(this.startAngle), z = this.labelsPosition == "outside", u = this.hiddenSlices, F = D
				- u.length, n = 0, d = 0; d < D; d++) {
			if (a.inArray(d, u) != -1) {
				n++;
				continue
			}
			...
			if (j == 0)
				continue;
			var G = this._getLabelText(v), e = (o + x) / 2, c = new m(G);
			c.textBaseline = "top";
	...
	h.prototype._arrangeLabels = function(c) {
		...
		for ( var f = [], e = [], u = this._calculateLabelsMargin(j, c), q = c.radius = this
				._calcRadius(u), a, t, o = n, m = n, b = 0; b < j.length; b++) {
			a = j[b];
			t = a.width;
			if (a.segment.isLeft) {
				f.push(a);
	...
	h.prototype.labelComparator = function(a) {
		a = a ? -1 : 1;
		return function(e, f) {
			var c = e.segment.angle, d = f.segment.angle, c = b
					.normalizeAngle(c + 3 * Math.PI / 2), d = b
					.normalizeAngle(d + 3 * Math.PI / 2);
			return (c - d) * a
		}
	};
	...
	h.prototype._renderSlices = function(A, z) {
		...
		if (!p)
			return;
		var f = this.chart.gridArea, D = f.width, C = f.height, n = this
				._getYValues(), q = n.length, r = this.getTotal(), m = this
				._calcRadius(z);
		if (m < 0)
			return;
		for ( var u = f.x + D / 2, v = f.y + C / 2, B = Math.PI * 2 / r, e = b
				.radians(this.startAngle), E = this.fillStyles, i = this.explodedRadius, k = this.hiddenSlices, x = q
				- k.length, j = 0, l, c = 0; c < q; c++) {
			if (a.inArray(c, k) != -1) {
				j++;
				continue
			}
	...
	h.prototype._calcRadius = function(b) {
		var f = b.v || this.margin, c = this.chart.gridArea, e = c.width - 2
				* b.h, d = c.height - 2 * f, a;
		if (e < d)
			a = e / 2;
		else
			a = d / 2;
	...
	h.prototype._addSliceAnimation = function(c, g, f) {
		...
		if (!b || b.enabled === false)
			return;
		var a = new tb(b, c, "endAngle", c.startAngle, c.endAngle), d = a.duration
				/ f, e = a.delayTime + g * d;
		a.delayTime = e;
		a.duration = d;
		this.chart.storyboard.addAnimation(a)
	...
	Ob.prototype._createShape = function(g, l) {
		...
		var i = this.chart.gridArea, p = i.y, o = i.y + i.height, h = this.realYAxis
				.getCrossingPosition();
	...
	v.prototype._render = function(t) {
		if (!this.hasRealData || !this.isInScene())
			return;
		for ( var j = this.arrData, A = this.chart.series._findStackedClusters(
				this, this.type), n = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), v = j.length, h = this.markers != null
				&& this.markers.isVisible(), k = [], r = [], s = [], p = [], x = h ? this.markers.size / 2
				: 0, y = this.xAxisType, l, m, b, g, d, f, c = 0; c < v; c++) {
			b = j[c];
			if (b == null)
				continue;
			...
			k.push(l);
			k.push(m);
			var e = b.actualValue, u = this._getStackedTotal(n, i), w = e > 0 ? u.positive
					: u.negative, o = w != 0 ? 100 * Math.abs(e) / Math.abs(w)
					: 0, z = {
				chart : this.chart,
				series : this,
				dataItem : b,
	...
	R.prototype._render = function(v) {
		if (!this.hasRealData || !this.isInScene())
			return;
		for ( var i = this.arrData, o = this.chart.series._findStackedClusters(
				this, this.type), f = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), u = this.xAxisType, w = i.length, j = [], k = [], n = [], l, r, s, a, e, d, x, h, b = 0; b < w; b++) {
			a = i[b];
			if (a == null)
				continue;
			...
			j.push(r);
			if (o.index != 0) {
				s = this._getPrevStackedPosition(f, o.index, c, 0,
						this.realYAxis, d >= 0);
				k.push(l);
				k.push(s)
			}
			var g = a.actualValue, p = this._getStackedTotal(f, c), q = g > 0 ? p.positive
					: p.negative, t = q != 0 ? 100 * Math.abs(g) / Math.abs(q)
					: 0;
			n.push({
				dataItem : a,
				index : b,
	...
	K.prototype._renderCatData = function(E) {
		...
		if (!s)
			return;
		var r = this.chart.gridArea, O = r.y, N = r.y + r.height, i = this.realYAxis
				.getCrossingPosition();
		i = b.fitInRange(i, O, N);
		i = Math.round(i);
		var d = this.chart.series._findStackedClusters(this, this.type), G = s.length, J = r.width
				/ this.realXAxis.getZoom(), C = J / G, M = C / d.groupCount, g = Math
				.round(this.pointWidth * M), I = d.groupCount * g, v = (C - I) / 2;
		v = Math.round(v + d.groupIndex * g);
		g = Math.max(g, this.minimumWidth);
		for ( var B = [], A = [], t = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), K = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 : 0, f, h, e, c = 0; c < G; c++) {
			e = s[c];
			if (e == null)
				continue;
			...
			var o = this._getPrevStackedPosition(t, d.index, q, i,
					this.realYAxis, m >= 0), w = h, p;
			if (h <= o)
				p = o - h;
			else {
				p = h - o;
			...
			var n = e.actualValue, F = this._getStackedTotal(t, q), H = n > 0 ? F.positive
					: F.negative, y = H != 0 ? 100 * Math.abs(n) / Math.abs(H)
					: 0, u = {
				chart : this.chart,
				series : this,
				dataItem : s[c],
			...
			if (this.labels && this.labels.visible !== false
					&& e.actualValue != 0
					&& this.realXAxis.isValueVisible(c + .5)) {
				var x = n;
				switch (this.labels.valueType) {
				case "percentage":
					x = y
				}
				var z = this.labels.position == "outside", L = z ? K : -p / 2, j = this
						._getDataPointLabel(e.actualValue, f, w, L, x);
				if (!z)
	...
	K.prototype._renderLinearData = function(C) {
		...
		if (!h)
			return;
		var s = this.chart.gridArea, M = s.y, J = s.y + s.height, i = this.realYAxis
				.getCrossingPosition();
		i = b.fitInRange(i, M, J);
		i = Math.round(i);
		var e = this.chart.series._findStackedClusters(this, this.type), n = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), L = h.length, F = s.width
				/ this.realXAxis.getZoom(), I = this._calcColumnScale(n) * F, t = this.pointWidth
				* I;
		t = Math.max(t, this.minimumWidth);
		for ( var A = [], z = [], G = this.markers && this.markers.isVisible() ? this.markers.size / 2
				: 0, f, g, c, d = 0; d < L; d++) {
			c = h[d];
			if (c == null)
				continue;
			...
			var p = this._getPrevStackedPosition(n, e.index, r, i,
					this.realYAxis, m >= 0), v = g, q;
			if (g <= p)
				q = p - g;
			else {
				q = g - p;
			...
			var o = c.actualValue, D = this._getStackedTotal(n, r), E = o > 0 ? D.positive
					: D.negative, x = E != 0 ? 100 * Math.abs(o) / Math.abs(E)
					: 0, u = {
				chart : this.chart,
				series : this,
				dataItem : h[d],
			...
			if (this.labels && this.labels.visible !== false
				...
				var y = this.labels.position == "outside", H = y ? G : -q / 2, j = this
						._getDataPointLabel(c.actualValue, f, v, H, w);
				if (!y)
					j.textBaseline = "middle";
				j.context = u;
				this.chart.elem.trigger("dataPointLabelCreating", j);
	...
	H.prototype._renderCatData = function(E) {
		...
		if (!r)
			return;
		var v = this.chart.gridArea, G = v.x, N = G + v.width, j = this.realYAxis
				.getCrossingPosition();
		j = b.fitInRange(j, G, N);
		j = Math.round(j);
		var d = this.chart.series._findStackedClusters(this, this.type), H = r.length, K = v.height
				/ this.realXAxis.getZoom(), C = K / H, O = C / d.groupCount, g = Math
				.round(this.pointWidth * O), J = d.groupCount * g, u = (C - J) / 2;
		u = Math.round(u + d.groupIndex * g);
		g = Math.max(g, this.minimumWidth);
		for ( var B = [], A = [], s = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), L = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 + 2 : 2, f, i, e, c = 0; c < H; c++) {
			e = r[c];
			if (e == null)
				continue;
			...
			var o = this._getPrevStackedPosition(s, d.index, p, j,
					this.realYAxis, m >= 0), w = f, q;
			if (f <= o)
				q = o - f;
			else {
				q = f - o;
			...
			var n = e.actualValue, F = this._getStackedTotal(s, p), I = n > 0 ? F.positive
					: F.negative, y = I != 0 ? 100 * Math.abs(n) / Math.abs(I)
					: 0, t = {
				chart : this.chart,
				series : this,
				dataItem : r[c],
			...
			if (this.labels && this.labels.visible !== false
				...
				var z = this.labels.position == "outside", M = z ? L : -q / 2, h = this
						._getDataPointLabel(e.actualValue, w, i, M, x);
				if (!z) {
	...
	H.prototype._renderLinearData = function(C) {
		...
		if (!h)
			return;
		var u = this.chart.gridArea, E = u.x, L = E + u.width, i = this.realYAxis
				.getCrossingPosition();
		i = b.fitInRange(i, E, L);
		i = Math.round(i);
		var s = this.chart.series._getStackedSeriesFromType(this.type,
				this.stackedGroupName), f = this.chart.series
				._findStackedClusters(this, this.type), N = h.length, G = u.height
				/ this.realXAxis.getZoom(), K = this._getSeriesFromThisType(), J = this
				._calcColumnScale(K)
				* G, r = this.pointWidth * J;
		r = Math.max(r, this.minimumWidth);
		for ( var A = [], z = [], H = this.markers && this.markers.isVisible() ? this.markers.size / 2 + 2
				: 2, e, m, c, d = 0; d < N; d++) {
			c = h[d];
			if (c == null)
				continue;
			...
			var o = this._getPrevStackedPosition(s, f.index, p, i,
					this.realYAxis, k >= 0), v = e, q;
			if (e <= o)
				q = o - e;
			else {
				q = e - o;
			...
			var n = c.actualValue, D = this._getStackedTotal(s, p), F = n > 0 ? D.positive
					: D.negative, x = F != 0 ? 100 * Math.abs(n) / Math.abs(F)
					: 0, t = {
				chart : this.chart,
				series : this,
				dataItem : h[d],
			...
			if (this.labels && this.labels.visible !== false
				...
				var y = this.labels.position == "outside", I = y ? H : -q / 2, g = this
						._getDataPointLabel(c.actualValue, v, m, I, w);
				if (!y) {
	...
	G.prototype._renderCatData = function(v) {
		var g = this.arrData, w = this.chart.gridArea, o = this.categories, p = o.length, r = w.width
				/ this.realXAxis.getZoom(), n = r / p, f = Math
				.round(this.pointWidth * n), u = Math.round((n - f) / 2);
		f = Math.max(f, this.minimumWidth);
		for ( var q, j, k, c, h, i, b = 0; b < g.length; b++) {
			...
			var s = {
				...
				from : h,
				to : i
			}, x = Math.min(j, k), t = Math.abs(j - k), d = new l(q, x, f, t);
			d.context = s;
	...
	G.prototype._renderLinearData = function(s) {
		var e = this.arrData, t = this.chart.gridArea, v = e.length, n = t.width
				/ this.realXAxis.getZoom(), r = this._getSeriesFromThisType(), o = this
				._calcColumnScale(r)
				* n, d = this.pointWidth * o;
		...
		for ( var m, j, k, i, g, h, b = 0; b < v; b++) {
			...
			var u = m - d / 2, w = Math.min(j, k), q = Math.abs(j - k), f = new l(
					u, w, d, q);
			f.context = p;
	...
	t.prototype._renderCatData = function(u) {
		var g = this.arrData, v = this.chart.gridArea, o = this.categories, p = o.length, r = v.height
				/ this.realXAxis.getZoom(), n = r / p, d = this.pointWidth * n, t = Math
				.round((n - d) / 2);
		d = Math.max(d, this.minimumWidth);
		for ( var j, k, q, c, h, i, b = 0; b < g.length; b++) {
			...
			var s = {
				...
				from : h,
				to : i
			}, x = Math.min(j, k), w = Math.abs(j - k), e = new l(x, q, w, d);
			e.context = s;
	...
	t.prototype._renderLinearData = function(s) {
		var f = this.arrData, t = this.chart.gridArea, m = f.length, o = t.height
				/ this.realXAxis.getZoom(), r = this._getSeriesFromThisType(), q = this
				._calcColumnScale(r)
				* o, e = this.pointWidth * q;
		...
		for ( var j, k, n, i, g, h, b = 0; b < m; b++) {
			...
			j = Math.round(this.realYAxis.getPosition(g));
			k = Math.round(this.realYAxis.getPosition(h));
			var v = n - e / 2, w = Math.min(j, k), u = Math.abs(j - k), d = new l(
					w, v, u, e);
			d.context = p;
			this._setShapeSettings(d, b);
			s.push(d);
	...
	V.prototype._getTooltip = function(b) {
		var d = this.realYAxis, a = d._getTooltip(b.from).replace("<br/>", "")
				+ " - " + d._getTooltip(b.to);
		if (this.title) {
			var e = c.getColorFromFillStyle(this.fillStyle);
	...
	F.prototype._processData = function() {
		this.arrData = null;
		if (this.data)
			if (!this.xValuesType && !this.highValuesType
					&& !this.lowValuesType && !this.openValuesType
			...
			var c = this.chart.arrDataSource;
			if (c) {
				var n = this.xValuesField, k = this.highValuesField, m = this.lowValuesField, l = this.openValuesField, j = this.closeValuesField, i = b
						.processDataField(c, n), f = b.processDataField(c, k), h = b
						.processDataField(c, m), g = b.processDataField(c, l), e = b
						.processDataField(c, j);
				if (i && f && h && g && e)
	...
	F.prototype._render = function(q) {
		if (!this.hasRealData || !this.isInScene())
			return;
		var n = this.arrData, r = this.chart.gridArea, j = n.length, d, g, i, h, e, l = r.width
				/ this.realXAxis.getZoom(), f;
		if (this.xAxisType == "CategoryAxis")
	...
	I.prototype._render = function(l) {
		...
		for ( var o = this.arrData, p = o.length, f = this.markers != null
				...
				&& this.labels.visible !== false, c = [], j = [], n = [], m = [], s = f ? this.markers.size / 2
				: 0, h, i, e, u = this.realYAxis.cx, q = this.realYAxis.cy, d = 0; d < p; d++) {
			var g = o[d];
			if (g === null) {
				c.push(null);
				c.push(null);
	...
	D.prototype._render = function(m) {
		if (!this.hasRealData || !this.isInScene())
			return;
		for ( var n = this.arrData, q = n.length, g = this.markers != null
				&& this.markers.isVisible(), w = this.labels != null
				&& this.labels.visible !== false, c = [], j = [], p = [], o = [], t = g ? this.markers.size / 2
				: 0, h, i, e, f, v = this.realYAxis.cx, r = this.realYAxis.cy, d = 0; d < q; d++) {
			var k = n[d];
			if (k === null) {
				c.push(null);
				c.push(null);
	...
	T.prototype._initData = function() {
		for ( var i = this.tData = this._getTrendlineResult(), f = j, e = n, d = j, c = n, k = i.length, h = 0; h < k; h++) {
			var g = i[h];
			if (g == null)
				continue;
			var a = g[0];
	...
	N.prototype._renderCatData = function(n) {
		for ( var o = this.arrData, p = o.length, f = this.markers != null
				&& this.markers.isVisible(), c = [], i = [], m = [], k = [], r = f ? this.markers.size / 2
				: 0, h, j, q, d, e = 0; e < p; e++) {
			var g = o[e];
			if (g === null) {
				c.push(null);
	...
	N.prototype._renderLinearData = function(o) {
		for ( var p = this.arrData, q = p.length, e = this.markers != null
				&& this.markers.isVisible(), s = this.labels != null
				&& this.labels.visible !== false, c = [], h = [], n = [], l = [], r = e ? this.markers.size / 2
				: 0, j, k, f, d, g = 0; g < q; g++) {
			var i = p[g];
			if (i === null) {
				c.push(null);
	...
	ib.prototype._createShape = function(i, f) {
		...
		var c = new q(h);
		this._setShapeSettings(c);
		var g = this.chart.gridArea, k = g.x, j = g.x + g.width, e = this.realYAxis
				.getCrossingPosition();
		e = b.fitInRange(e, k, j);
	...
	y.prototype._renderCatData = function(r) {
		for ( var s = this.arrData, k = s.length, e = this.markers != null
				&& this.markers.isVisible(), c = [], d = [], f = [], q = [], o = [], t = e ? this.markers.size / 2
				: 0, j, m, n, l, g, h, b = 0; b < k; b++) {
			var i = s[b];
			if (i === null) {
				c.push(null);
				c.push(null);
	...
	y.prototype._renderLinearData = function(r) {
		for ( var s = this.arrData, k = s.length, d = this.markers != null
				&& this.markers.isVisible(), u = this.labels != null
				&& this.labels.visible !== false, i = [], j = [], e = [], q = [], o = [], t = d ? this.markers.size / 2
				: 0, h, m, n, l, f, g, b = 0; b < k; b++) {
			var c = s[b];
			if (c === null) {
				pts.push(null);
				pts.push(null);
	...
	y.prototype._addMarkersAndLabels = function(g, n, i, p, q, a, h, o, d, e, m) {
		...
		if (this.labels && this.labels.visible !== false) {
			var j = this._getLabelValue(d, a), c = this._getDataPointLabel(d,
					i, p, m + k, j, d > e), f = {
				chart : this.chart,
				series : this,
				dataItem : data[a],
	...
	y.prototype._getDataPointLabel = function(j, i, e, c, f, g) {
		var h = this._getLabelText(f), b = new m(h);
		d.setShadows(b, this, this.chart);
		a.extend(b, this.labels);
		b.measure(this.chart.ctx);
		b.textAlign = "center";
	...
	o.prototype._arrange = function() {
		...
		if (!this.jqRangeSlider) {
			var d = a('<div style="position:absolute"></div>')
					.jqRangeSlider({}), e = this, f = e.chart.options;
	...
	o.prototype._mouseWheelScroll = function(d) {
		var c = this.actualVisibleMinimum, b = this.actualVisibleMaximum, a = d
				* (this.actualMaximum - this.actualMinimum) / 20;
	...
	p.prototype._initRange = function() {
		...
		var o = 0, m = 0, f = i._getPixelMargins(this);
		if (this.isAxisVertical) {
			f.left = b.isNull(this.bottomMargin) ? f.left + .5
					: this.bottomMargin;
			f.right = b.isNull(this.topMargin) ? f.right + .5 : this.topMargin
		} else {
			f.left = b.isNull(this.leftMargin) ? f.left + .5 : this.leftMargin;
			f.right = b.isNull(this.rightMargin) ? f.right + .5
					: this.rightMargin
		}
		var q = h / this.length;
		o = q * f.left;
		...
		var a = e - o, c = d + m, t = this.series._isAnchoredToOrigin(), g = this
				.getCrossing();
	...
	p.prototype._getIntervals = function(c, a, h) {
		...
		if (a && a.intervalOffset)
			e = a.intervalOffset;
		for ( var f = [], g = this._getIntervalStart(this
				._getActualVisibleMinimum(), c), d = g + e; d <= this
				._getActualVisibleMaximum(); d = b.round(d + c))
	...
	k.prototype._getIntervals = function(e, c) {
		if (this.customTickMarks)
			return this.customTickMarks;
		var g = [], h = this.actualVisibleMinimum, j = this.actualVisibleMaximum, a = this
				._getIntervalStart(h, e, this.actualIntervalType);
		while (a < h)
		...
		for ( var b = a; b <= j; b = this._incrementDateTime(b, e,
	...
	k.prototype._getNextNonEmptyDay = function(b) {
		...
		if (d == -1)
			return b;
		for ( var f = g.addDays(new Date(b), 1), h = this.actualVisibleMaximum, c = f; c <= h; c = g
				.addDays(c, 1)) {
			var e = c.getTime();
	...
	w.prototype._getLabels = function() {
		...
		if (a == null || a.visible === false)
			return [];
		for ( var m = this._getMaxOutsideTickMarksLength() + this.lblMargin, e = [], n = this
				._getMarkInterval(a, true), f = this._getLabelIntervals(n, a), g = f.length, j = a.showFirstLabel, k = a.showLastLabel, c = 0; c < g; c++) {
			if (!j && c == 0 || !k && c == g - 1)
				continue;
			var h = f[c], l = this.getLabel(h), d = this._createLabel(l, a), o = this
					.getPosition(h), q = this.cy, p = o, i = b.rotatePointAt(p,
					q, -Math.PI / 2, this.cx, this.cy);
			d.x = i.x - m;
			d.y = i.y;
			d.textAlign = "right";
			e.push(d)
	...
	w.prototype._getTickMarks = function(a, l) {
		if (a == null || a.visible != true)
			return [];
		for ( var h = [], s = this._getMarkInterval(a, l), t = a.length, k = this
				._getIntervals(s, a, l), g = this.sharedAxis, f = g
				._getIntervals(g.actualInterval), p, r, q, c, d = 0; d < k.length; d++) {
			var u = this.getPosition(k[d]);
			p = q = u;
			c = this.cy;
			r = c - t;
			for ( var e = 0; e < f.length; e++) {
				var v = f[e], j = this.sharedAxis._getAngle(v), n = b
						.rotatePointAt(p, r, j, this.cx, this.cy), o = b
						.rotatePointAt(q, c, j, this.cx, this.cy), m = new i(
						n.x, n.y, o.x, o.y);
	...
	w.prototype._createRenderShape = function(f, g, e) {
		...
		if (this.renderStyle != "polygon") {
			if (!b.isNull(a.startAngle) && !b.isNull(a.endAngle)) {
				var h = b.radians(a.startAngle), i = b.radians(a.endAngle), m = i
						- h;
		...
		for ( var c = [], j = this.sharedAxis.actualMaximum, o = 2 * Math.PI
				/ j, f = this.cx, g = this.cy - e, d = 0; d < j; d++) {
	...
	L.prototype._render = function(n) {
		...
		for ( var d = this.cx, e = this.cy, q = d, s = e
				...
				* Math.PI / g, h = 0; h < g; h++) {
			var k = h * p, l = b.rotatePointAt(q, s, k, d, e), m = b
					.rotatePointAt(r, t, k, d, e), c = new i(l.x, l.y, m.x, m.y);
			c.strokeStyle = this.strokeStyle;
			c.lineWidth = this.lineWidth;
			c.strokeDashArray = this.strokeDashArray;
			if (o)
	...
	C.prototype._render = function(o) {
		...
		for ( var t = d, u = e - this.radius, p = this.renderLinesOverGraph, k = this
				._getIntervals(this.actualInterval), q = this
				._getIntervalsLength(k), g = 0; g < q; g++) {
			var r = k[g], j = this._getAngle(r) + Math.PI / 2, l = b
					.rotatePointAt(s, n, j, d, e), m = b.rotatePointAt(t, u, j,
					d, e), c = new i(l.x, l.y, m.x, m.y);
			c.strokeStyle = this.strokeStyle;
			c.lineWidth = this.lineWidth;
			c.strokeDashArray = this.strokeDashArray;
			if (p)
	...
	ub.prototype.getUpperError = function() {
		...
		return a == "both" || a == "upper" ? this.calculationType == "percentage" ? this.errorAmount.upper
				: this.errorAmount
				: 0
	};
	ub.prototype.getLowerError = function() {
		var a = this.displayType;
		return a == "both" || a == "lower" ? this.calculationType == "percentage" ? this.errorAmount.lower
				: this.errorAmount
				: 0
	};
